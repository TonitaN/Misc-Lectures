...
В этом варианте по каждой переменной без учёта рестрикций ровно два случая:
\begin{itemize}
\item сжатие в единственный блок $X \mapsto a^i$;
\item извлечение двух блоков слева и справа, $X \mapsto a^{i_1} X a^{i_2}$, с дополнительными условиями $X\neq\varepsilon$, $\mathtt{(not\; a\; starts\;X)}$, $\mathtt{(not\; a\; ends\;X)}$.
\end{itemize} 

Напомним, что в дальнейшем все различные блоки (собранные из идущих подряд блоков $A$) переводятся в различные новые буквы. Например: выражение $a_0a_0^{i_1}X a_0^{i_1}a_0^{i_2}X a_0^{i_2}a_0a_0$ индуцирует замены $a_1 := a_0^{i_1 +1}$, $b_1 := a_0^{i_1 +i_2}$, $c_1 := a_0^{i_2 +2}$. В итоге выражение будет представлено как $a_1 X b_1 X c_1$, а указанные выше подстановки попадут в блок уравнений на индексы.
...
Можно было бы возразить, что такая рестрикция может всё равно неявно появиться, если сжатие уже не первое (сжали блок $a_0$, получили $a_1$; затем сжали блок $b_0$, получили $b_1$; сжали пару $a_1 b_1$, получили $c_1$, но если $a_1$ и $b_1$ оба были нулевыми степенями букв, то извлечение блоков $c_1$ заведомо абсурдно). Второй похожий вопрос состоит в том, что если $a_1$ и $b_1$ оба являются блоками $a_0$, то допускать ли возможность, что пары $a_1 b_0$ и $b_1 b_0$ должны сжиматься в одну и ту же константу.
...
На практике эти соглашения просто означают, что никакие дополнительные проверки на равенство и пустоту не нужны. 
...
Симметрично определим понятие $\mathrm{Last}(\alpha)$. В дальнейшем будем оперировать с понятием $\mathrm{First}$, по умолчанию считая, что для двойственного ему все действия и утверждения будут симметричны.
...
( 
 (('B' 1) is ('B' (i1 1) (const 1))) 
 (('B' 2) is ('B' (i2 1) (const 2))) 
)
\end{minted}
\item В блок коллапсирует только \verb|X|:
\begin{minted}[breaklines,breakafter= ]{refal}
